...
async function getInventoryItems(): Promise<{ items: InventoryItem[]; error?: string }> {
  ...
  try {
    ...
    const inventoryCollectionRef = collection(db, 'inventory');
     // Query to order items by creation date, newest first
...
export default async function InventoryPage() {
  ...
  // Extract unique styles and materials for filters (consider moving logic client-side for dynamic filtering)
  ...
  return (
    <div className="container mx-auto py-6 animate-subtle-fade-in space-y-8"> {/* Added spacing */}
      ...
      {/* Filters and Search Section - Placeholder: Implement client-side filtering for interactivity */}
      <Card className="p-4 md:p-6 shadow-sm">
        <div className="flex flex-col md:flex-row gap-4 items-center">
          <div className="relative flex-grow w-full md:w-auto">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search by name or SKU... (client-side)" className="pl-10" disabled />
          </div>
